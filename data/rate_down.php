...
if($user != ""){
	if($_POST['id']){
		...
		//Verify IP address in rating_ip table
		$ip_sql=mysql_query("SELECT bind_user FROM rating_ip WHERE sig_id_fk='$id' AND bind_user='$user'");
		$count=mysql_num_rows($ip_sql);
		
		if($count==0){
			// update rating
			$sql = "UPDATE sighting SET rating_down=rating_down+1 WHERE sighting_id='$id'";
			mysql_query( $sql);
			// insert IP address and sighting id in rating_ip table
		...
		$result = mysql_query("SELECT rating_down FROM sighting WHERE sighting_id='$id'");
		$row = mysql_fetch_array($result);
		$down_value = $row['rating_down'];
		echo $down_value;
		}
	}
	else{
